...
outd <- args[3]
overlap <- args[4] ## 1 or 0, TRUE or FALSE: requiring targets in the same gene or not
...
inputFile1 <- paste(wds[1], "/", targets[1], ".dist.measures.txt", sep="")
inputFile2 <- paste(wds[2], "/", targets[2], ".dist.measures.txt", sep="")
...
print(paste(targets[1], "peaks table"))
dim(m6a.dist)
...
if(overlap){
	# limit pseudoU data to those overlap with m6a-containing genes
	pseudoU.dist <- pseudoU.dist[pseudoU.dist$refseqID %in% m6a.dist$refseqID,]
}

print(paste(targets[2], "peak table"))
dim(pseudoU.dist)
...
peakType <- c(rep(targets[1], length(m6a.metagene.coord)), 
         rep(targets[2], length(pseudoU.metagene.coord))) 
df <- data.frame(metagene.cord, peakType)

print("length of second peaks")
print(length(pseudoU.metagene.coord))

print("length of first peaks")
print(length(m6a.metagene.coord))

#fact <- unlist(strsplit(targets[1], "_", fixed=T))[3]
#fact <- unlist(strsplit(fact, ".", fixed=T))[1]

ggplot(df) + geom_density(aes(x = metagene.cord, colour = peakType), size = 1) + xlim(0, 3) + 
  theme_bw() + geom_vline(xintercept = 1:2, col = "grey") + ggtitle("metagene plot") + theme(
    plot.title = element_text(color="red", size=24, face="bold.italic"))


ggsave(paste(targets[1], "_", targets[2], "_metaPlotR.png", sep=""), plot = last_plot(), device = "png", path = outd,
       scale = 1, width = 8, height = 8, units = "in",
       dpi = 300, limitsize = TRUE)

print(paste("finish metaPlotR for", targets[1], targets[2]))
